...
# 1. 设置当前会话的环境变量
echo "1️⃣ 设置环境变量..."
source "$PROJECT_DIR/scripts/proxy-on.sh"
echo ""
...
# 7. 运行检查
echo "7️⃣ 运行代理检查..."
bash "$PROJECT_DIR/scripts/check-proxy.sh"
echo ""

echo "🎉 代理配置完成！"
echo ""
echo "📝 使用说明:"
echo "  开启代理: source scripts/proxy-on.sh 或 proxy-on"
echo "  关闭代理: source scripts/proxy-off.sh 或 proxy-off"
echo "  检查状态: bash scripts/check-proxy.sh"
